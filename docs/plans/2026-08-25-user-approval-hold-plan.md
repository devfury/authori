...
- [x] 7. 웹 API 클라이언트·StatusBadge: 타입/메서드 추가, 배지 키 추가
- [x] 8. `HoldUsersDialog.vue` 신규 작성
- [x] 9. `UserListView.vue`: 체크박스 선택, 일괄 액션 바, 확인 다이얼로그 연결, 결과 요약, 보조 배지, 승인 대기 필터(URL 동기화 포함)
- [x] 10. `UserDetailView.vue`: 배지·시각 표시, 단건 보류 버튼
- [x] 11. CLAUDE.md 승인 대기 알림 단락 갱신
- [x] 12. 4단계 검증 실행 및 통과 확인
