...
백엔드 서비스가 사용자 역할을 프로그램 방식으로 조회·변경해야 하는 경우(예: 관리자 승인 후 역할 부여, JIT 역할 프로비저닝), M2M RBAC API를 사용합니다. 이 API는 `client_credentials` grant로 발급한 M2M 액세스 토큰으로 호출합니다.
...
단계 (4) 토큰 교환 후 백엔드 서비스가 access token JWT를 복호화하거나, 단계 (5) userinfo 응답 대신 토큰 클레임에서 직접 역할/권한을 읽어 서비스 내 접근 제어에 활용합니다.
