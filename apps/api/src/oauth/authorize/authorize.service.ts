...
import { DEFAULT_TENANT_SCOPES, ScopesService } from '../scopes/scopes.service';
...
export class AuthorizeService {
  ...
  async initiateAuthorize(tenantId: string, tenantSlug: string, query: AuthorizeQueryDto) {
    ...
    const scopeDetails = await this.scopesService.getScopeDetails(tenantId, requestedScopes);
  ...
  async getLoginConfig(tenantId: string, clientId?: string) {
    ...
    const defaultScopeByName = new Map(DEFAULT_TENANT_SCOPES.map((scope) => [scope.name, scope]));
  ...
  async loginAndAuthorize(tenantId: string, dto: LoginAuthorizeDto, ctx: AuditContext = {}) {
    ...
    if (provider) {
      ...
      if (provider.syncOnLogin && result.user) {
        const mapped = this.externalAuthService.applyFieldMapping(result, provider.fieldMapping);
    ...
    // ── 로컬 전용 경로 ──────────────────────────────────
    if (!user) {
      await this.recordAuditLoginFailure(tenantId, pending.clientId, null, 'user_not_found', ctx, {
        email: dto.email,
      });
      throw new UnauthorizedException('invalid_credentials');
    }
  ...
  private async issueAuthCode(
    ...
    if (dto.grantedScopes) {
      const overGranted = dto.grantedScopes.filter((scope) => !pending.scopes.includes(scope));
      if (overGranted.length > 0) {
        throw new BadRequestException(`invalid_scope: ${overGranted.join(', ')}`);
      }
    }

    const grantedScopes = Array.from(new Set(dto.grantedScopes ?? pending.scopes));
