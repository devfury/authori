...
export class ScopesService {
  ...
  async seedDefaults(tenantId: string): Promise<void> {
    ...
    const missing = DEFAULT_TENANT_SCOPES.filter((scope) => !existingNames.has(scope.name)).map(
      (scope) => this.scopeRepo.create({ tenantId, ...scope }),
    );
  ...
  async getScopeDetails(tenantId: string, names: string[]): Promise<TenantScope[]> {
  ...
  async create(tenantId: string, dto: CreateScopeDto): Promise<TenantScope> {
    ...
    return this.scopeRepo.save(this.scopeRepo.create({ tenantId, ...dto, isDefault: false }));
  }
