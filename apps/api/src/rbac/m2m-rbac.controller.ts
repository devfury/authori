...
export class M2mRbacController {
  ...
  findUserRoles(@CurrentTenant() tenant: TenantContext, @Param('userId') userId: string) {
  ...
  async addUserRole(
    ...
    const roles = await this.rbacService.addUserRole(tenant.tenantId, userId, roleId);
  ...
  async removeUserRole(
    ...
    const roles = await this.rbacService.removeUserRole(tenant.tenantId, userId, roleId);
  ...
  async setUserRoles(
    ...
    const roles = await this.rbacService.setUserRoles(tenant.tenantId, userId, dto.roleIds);
    await this.recordRoleAudit(req, tenant.tenantId, userId, undefined, 'replace', {
      roleIds: dto.roleIds,
    });
    return roles;
  }
