...
import { RolePermission, TenantPermission, TenantRole, User, UserRole } from '../database/entities';
...
export class RbacService {
  ...
  async updateRole(tenantId: string, id: string, dto: UpdateRoleDto): Promise<TenantRole> {
  ...
  async findPermission(tenantId: string, id: string): Promise<TenantPermission> {
  ...
  async createPermission(tenantId: string, dto: CreatePermissionDto): Promise<TenantPermission> {
  ...
  async setUserRoles(tenantId: string, userId: string, roleIds: string[]): Promise<TenantRole[]> {
    await this.assertUserBelongsToTenant(tenantId, userId);
  ...
  async addUserRole(tenantId: string, userId: string, roleId: string): Promise<TenantRole[]> {
    await this.assertUserBelongsToTenant(tenantId, userId);
  ...
  async removeUserRole(tenantId: string, userId: string, roleId: string): Promise<TenantRole[]> {
    await this.assertUserBelongsToTenant(tenantId, userId);
    await this.assertRolesBelongToTenant(tenantId, [roleId]);
  ...
  async assignDefaultRolesToUser(tenantId: string, userId: string): Promise<TenantRole[]> {
    await this.assertUserBelongsToTenant(tenantId, userId);
  ...
  async getUserPermissionNames(tenantId: string, userId: string): Promise<string[]> {
