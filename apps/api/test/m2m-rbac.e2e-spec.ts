...
describe('M2M RBAC API (e2e)', () => {
  ...
  it('lists tenant roles and user roles with rbac:read', async () => {
    await dataSource.getRepository(UserRole).save({ userId, roleId: adminRoleId });
    ...
    const roles = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(roles.body.map((role: { id: string }) => role.id)).toEqual([adminRoleId, viewerRoleId]);

    const userRoles = await request(app.getHttpServer())
      .get(`/t/${tenantSlug}/api/users/${userId}/roles`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200);
    expect(userRoles.body.map((role: { id: string }) => role.id)).toEqual([adminRoleId]);
  });
  ...
  it('adds and removes a user role idempotently with rbac:write', async () => {
    ...
    expect(await dataSource.getRepository(UserRole).countBy({ userId })).toBe(1);
    ...
    expect(await dataSource.getRepository(UserRole).countBy({ userId })).toBe(0);
  ...
  it('replaces user roles with rbac:write', async () => {
    ...
    expect(res.body.map((role: { id: string }) => role.id)).toEqual([viewerRoleId]);
  });
