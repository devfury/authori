  ...
  <div>
    ...
    <div class="mb-4 flex flex-col lg:flex-row gap-3">
      ...
      <div class="flex gap-2">
        <select
          v-model="roleFilter"
          class="block w-full lg:w-40 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 bg-white"
        ...
        <select
          ...
          class="block w-full lg:w-32 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 bg-white"
          @change="onFilterChange"
        >
          <option value="">상태 (전체)</option>
          <option :value="AdminStatus.ACTIVE">활성</option>
          <option :value="AdminStatus.INACTIVE">비활성</option>
        </select>
        <button
          @click="refresh"
          :disabled="loading"
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div v-if="loading && admins.length === 0" class="p-8 text-center text-sm text-gray-400">불러오는 중...</div>
